...
- `build_assetbundle.ps1` — Unity 2019.4.40f1 build, graphical verification and LZMA repack of the main, winter and track AssetBundles.
...
- `generate_dvseasons_texture_pack.py` — generator for the original 90-texture terrain/vegetation subset; the current authoritative 173 PNG inputs are stored in `DVSeasons.Unity/Assets/DVSeasons/DV99`.
- `prepare_winter_surface_textures.py` — fits the supplied road/sidewalk images to DV99's exact source dimensions, restores original alpha, and converts the generated ice reference into a periodic albedo. Generated authoring PNGs are compiled into the winter bundle.
- `generate_ice_normal.py` — deterministic standard-library generator for the seamless linear winter-water normal map. The runtime uses the prepared bundled texture.
