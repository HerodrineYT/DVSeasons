...
param(
    ...
    [string]$ModDirectory,
    [string]$LogName='thermal-sync-runtime.log'
)
...
& $compiler /nologo /nostdlib /target:library /langversion:7.3 "/out:$verification/VerifyThermalSync.dll" @refs (Join-Path $PSScriptRoot 'VerifyThermalSync.cs')
if($LASTEXITCODE -ne 0){throw 'Thermal sync fixture compilation failed.'}
...
try {
    ...
    $args=@('-batchmode','-projectPath',('"{0}"' -f (Join-Path $root 'DVSeasons.Unity')),
        '-executeMethod','DVSeasons.AssetBundleBuild.ThermalSyncVerification.Run','-logFile',('"{0}"' -f $log))
    $process=Start-Process -FilePath $UnityEditor -ArgumentList $args -WindowStyle Hidden -PassThru
    if(-not $process.WaitForExit(180000)){$process.Kill();throw 'Thermal sync fixture timed out.'}
    $process.WaitForExit()
    Select-String -LiteralPath $log -Pattern 'THERMAL_SYNC_.*|Exception:.*|error CS.*' | ForEach-Object {$_.Line}
    if($process.ExitCode -ne 0){throw "Thermal sync fixture failed. Log: $log"}
