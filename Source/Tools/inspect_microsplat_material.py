        ...
        for property_name, value in material["m_SavedProperties"]["m_TexEnvs"]:
            ...
            if file_id == 0 and path_id in objects:
                ...
                try:
                    ...
                    description = "{}:{} {}x{} depth={} format={}".format(
                        target.type.name,
                        data.get("m_Name"),
                        data.get("m_Width", ""),
                        data.get("m_Height", ""),
                        data.get("m_Depth", ""),
                        data.get("m_TextureFormat", ""),
                    )
                except Exception:
                    description = target.type.name
            print(property_name, "file", file_id, "path", path_id, "=>", description)
        print("FLOATS")
        for property_name, value in material.get("m_SavedProperties", {}).get("m_Floats", []):
            print(property_name, value)
        print("COLORS")
        for property_name, value in material.get("m_SavedProperties", {}).get("m_Colors", []):
            print(property_name, value)
        print("KEYWORDS", material.get("m_ShaderKeywords", ""))
        return 0
