...
В GitHub-пакет входят C#-проекты, тесты, документация, инструменты, визуальные эталоны и полный Unity-проект с текстурами. Из релиза исключены кэши, результаты сборки, PDB и вложенные архивы. Каждый из трёх готовых AssetBundle хранится в пакете один раз. Обычные зимние PNG находятся в исходном Unity-проекте; runtime получает готовые текстуры из bundle. Пользовательские замены устанавливаются в `Overrides/Seasonal`.
